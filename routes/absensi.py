...
def clock_out():
    try:
        ...
        # Cari data hari ini milik karyawan ini yang clock_out nya masih "-"(belum selesai)
    ...
    try:
        ...
        for doc in docs:
            ...
            if tanggal_absen.startswith(bulan_dicari):
                ...
                # Hitung Kehadiran (menggunakan Set agar tanggal sama tidak dihitung 2x)
                if jenis == 'Reguler':
                    laporan[emp_id]["dates"].add(tanggal_absen)
                
                # Hitung Berapa kali lembur
                if jenis == 'Lembur':
